...
//end exercise
    ...
    private function test_url($args)
    {
        ...
        for ($i = 1; $i <= $args->count; $i++) {
            ...
            WP_CLI::line(sprintf(
                            "%d\t%sK\t%s\t%d\t%s\t%s\t%s", $runs[$i]->response_code, number_format($runs[$i]->response_size / 1024, 1), number_format($runs[$i]->response_time, 2), $runs[$i]->response_cookies, $runs[$i]->response_modified, $runs[$i]->response_cachecontrol, $runs[$i]->response_canonical
            ));
        }//end for
